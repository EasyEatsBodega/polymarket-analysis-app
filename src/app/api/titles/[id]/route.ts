...
export async function GET(
  ...
  try {
    ...
    const globalRanks = title.weeklyGlobal.map((w) => ({
    ...
    const usRanks = title.weeklyUS.map((w) => ({
      week: w.weekStart.toISOString(),
      rank: w.rank,
      category: w.category,
    }));
    ...
    // Format forecasts
    const forecasts = title.forecasts.map((f) => {
    ...
    // Format market links
    const markets = title.marketLinks.map((link) => ({
      id: link.market.id,
